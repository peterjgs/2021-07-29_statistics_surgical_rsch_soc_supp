...
Remove data points further than 3 x SD out.


```{r}

jk_mean <- mean(jerk_mean2$MeanJerk, na.rm = T)

jk_mean_sd <- sd(jerk_mean2$MeanJerk, na.rm = T)

jk_mean_lb <- jk_mean - 3 * jk_mean_sd

jk_mean_ub <- jk_mean + 3 * jk_mean_sd

jerk_mean3 <- 
    jerk_mean2 %>%
    filter(MeanJerk >= jk_mean_lb & MeanJerk <= jk_mean_ub)
...
ggplot(jerk_mean3, aes(x = "", y = MeanJerk)) +
  geom_boxplot(outlier.shape = NA, width = 0.4) +  # avoid plotting outliers twice
  geom_jitter(position = position_jitter(width = 0.1, height = 0))
...
jerk.lm1 <- lm(MeanJerk ~ Condition2 * Trial2, data = jerk_mean3)
...
AIC(err.lm2)  ##  Gaus. - log trans.

AIC(err.glm1)  ##  Poisson

AIC(err.glm2)  ##  Neg. bin.
